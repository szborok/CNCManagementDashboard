...
export class DashboardDataService {
  static async loadDashboardData(): Promise<DashboardData | null> {
    try {
      ...
      const backendData =
        await BackendDataLoader.generateDashboardFromBackends();
      if (backendData) {
      ...
      recentActivity: [
        {
          ...
          details:
            "CNC Management Dashboard has been configured and is ready to use",
        },
      ],
      charts: {
        projectCompletion: [
  ...
  static async updateModuleStatus(
    module: keyof DashboardData["modules"],
    status: "active" | "disabled"
  ): Promise<void> {
    const currentData = await this.loadDashboardData();
    if (currentData) {
